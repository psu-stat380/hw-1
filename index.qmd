...
```{R}
#  The vector contains the data type 'Character'
```
...
sum(my_vec_double > 0)
# There are 4 elements of my_vec_double that are greater than zero
```
...
```{R}
# Between 'col_wise_scan' and 'row_wise_scan', I expect 'col_wise_scan' to take shorter to run, because it aligns more favorably with the memory cache and hence is cache efficient.
```
...
```{R}
# When we repeat the experiment for matrices of different sizes the time taken by 'row_wise_scan' and 'col_wise_scan' increases. We can also see that the time taken by 'row_wise_scan' increases at a slower rate than 'col_wise_scan'. 
```
