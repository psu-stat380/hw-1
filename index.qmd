---
title: "Homework 1"
author: "[Jack Kociolek]{style='background-color: yellow;'}"
toc: true
...
## 4 elements of the double vector are greater than zero, in our boolean vector these are true
...
> matrix1 <- matrix(mat1.data,nrow=3,ncol=3,byrow=TRUE)
> matrix1
```
...
```{R}
matrix2.data <- c(1,2,3,4,5,6,7,8,9)
> matrix2 <- matrix(mat1.data,nrow=2,ncol=100,byrow=TRUE)
> matrix2
```
...
## this function iterates through the matrix one row at a time, the final if statement increases the count if the entry is above 0
...
## I expect that they would take the same amount of time in a n x n matrix but if the amount of columns is much larger than rows, then row_wise_scan would be faster. The same applies if there are many more rows than columns.
