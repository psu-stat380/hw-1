...
4.  Sort the values of `my_vec_double` in ascending order.
    ...
    ```{R}
    library(tidyverse)
    # 1. The vector contains numeric data.
    typeof(my_vec)
    # 2.
    ...
    # 3.
    my_vec_bool <- ifelse(my_vec_double,"r True","r False")
    # 4.
    my_vec_double <- my_vec_double%>%
      sort()
    ```
...
4.  Between `col_wise_scan` and `row_wise_scan`, which function do you
    ...
    Column wise scan should be much faster. Because 64 bit cache line
    hold 16 integers, and doing it row wise would cause cache eviction,
    resulting in requiring it to be taken from the ram, which is a
    lengthier process.
...
Scanning a matrix column wise is more efficient.

The larger the matrix, the longer it takes.
