---
title: "Homework 1"
author: "[Brian Montagna]{style='background-color: yellow;'}"
toc: true
...
3. Create a new vector `my_vec_bool` which comprises of:
 ...
 ```{R}
my_vec_bool <- 'empty'
for(x in my_vec_double){
    if(x <= 0){
        x = 'TRUE'
        my_vec_bool <- c(my_vec_bool, x)
    }else{
        x = 'FALSE'
        my_vec_bool <- c(my_vec_bool, x)
    }
}
my_vec_bool <- my_vec_bool[! my_vec_bool %in% c('empty')]
my_vec_bool
```
...
I think row_wise_scan will take shorter to run because it is easier to scan across a matrix then down each collumn.
...
Col_wise_scan took longer to run
...
I can conclude that the more rows that you add the longer it takes for the scans to run.
