...
```{R}
"I expect col_wise_scan to take shorter to run because of the way that R generated matrices. R creates matrices by column unless specified otherwise."
```
...
```{R}
"I can conclude that initially with the 50x50 matrix, row scan took longer. However, as we kept increasing the size of the matrix row scan time difference started to decrease dramatically below the time to do column wise scan, meanwhile column wise scan time difference remained relatively the same."
```
