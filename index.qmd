---
title: "Homework 1"
author: "[Victoria Ritorto]{style='background-color: yellow;'}"
toc: true
...
```{R}
# The vector contains double and integer. 
```
...
y <- matrix(seq(1,100),nrow=1)
z <- matrix(seq(1,100),nrow=1)
# 1 to 1000, xn=n^2
a = rbind(y, z)
a
```
...
```{R}
# I expect col_wise_scan to take shorter to run because the data for each column is sorted seperately.  
```
...
```{R}
# I can conclude the larger matrix takes longer to run. 
```
