---
title: "Homework 1"
author: "Sung Chang {style='background-color: yellow;'}"
toc: true
...
```{R}
#I expect the row_wise_scan to run faster than the column counterpart because typically accessing elements in a row is easier since elements are subsequent while columns need to jump around more.
```
...
```{R}
#The row function took longer to run.
```
...
```{R}
#It seems like the row function still takes longer in all the experiments.
```
...
This resulted in TRUE but I could not compile pdf and html due to an error with %>%.
#```{R}
